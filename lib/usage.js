...
// 默认落系统临时目录下的 fengmiantu/（跟 jianji 的 WORK_DIR 同套路：os.tmpdir()）——
// 因为部署常把 /opt/fengmiantu 设成只读(systemd ProtectSystem / 只读挂载)，写不进
// __dirname/../data 会 EROFS，埋点静默失败、报表永远空。tmp 一定可写，开箱即用。
// 想持久化(重启不丢)就用 DATA_DIR 指到一个可写的持久目录（如挂载卷 / systemd StateDirectory）。
const DEFAULT_DIR = process.env.DATA_DIR || path.join(os.tmpdir(), 'fengmiantu');
...
// 按【马来西亚时区 UTC+8，无夏令时】取日期：ts 是 UTC ISO，+8h 后取 UTC 日期即当地日期。
// 否则凌晨(当地 00:00~07:59)的事件会被算到前一天(UTC)。
const dayOf = (ts) => {
  const d = new Date(ts);
  return Number.isNaN(d.getTime()) ? '未知' : new Date(d.getTime() + 8 * 3600000).toISOString().slice(0, 10);
};
...
module.exports = { logUsage, readUsage, aggregate, modeLabel, filePath, DEFAULT_DIR, dayOf };
