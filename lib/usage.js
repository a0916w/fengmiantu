...
function logUsage(event, opts = {}) {
  try {
    ...
    const rec = {
      ts: now.toISOString(),
      type: event.type === 'frame' ? 'frame' : 'publish',
    };
    if (event.type !== 'frame') {
...
/**
 * 聚合成报表所需结构：
 *  - days:   [{ date, frames, publishes }]      每天：截图生成数 / 用作封面总数（日期倒序）
 *  - detail: [{ date, mode, label, logo, count }] 用作封面明细：按 天/模式/logo（日期倒序）
 *  - totals: { frames, publishes }              全区间合计
 */
function aggregate(events) {
  const days = new Map();   // date -> { date, frames, publishes }
  const detail = new Map(); // date||mode||logo -> { date, mode, logo, count }
  let frames = 0, publishes = 0;

  for (const e of events || []) {
    const d = dayOf(e.ts);
    if (!days.has(d)) days.set(d, { date: d, frames: 0, publishes: 0 });
    const day = days.get(d);
    if (e.type === 'frame') {
      day.frames++; frames++;
    } else {
      day.publishes++; publishes++;
  ...
  return { days: dayRows, detail: detailRows, totals: { frames, publishes } };
}
