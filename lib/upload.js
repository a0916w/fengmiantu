...
function uploadTargetConfig(key) {
  if (key === 'default') return defaultTargetConfig();
  const K = envKey(key);
...
// 默认（兜底）上传目标：logo 没配专属 FTP 时用它。
// 优先 UPLOAD_DEFAULT_*；没配则退回现有的 FTP_*（webmm 那台“正常”FTP）。
function defaultTargetConfig() {
  const host = process.env.UPLOAD_DEFAULT_FTP_HOST || process.env.FTP_HOST;
  const user = process.env.UPLOAD_DEFAULT_FTP_USER || process.env.FTP_USER;
  if (!host || !user) return null;
  return {
    key: 'default',
    label: process.env.UPLOAD_DEFAULT_LABEL || '默认',
    host,
    port: parseInt(process.env.UPLOAD_DEFAULT_FTP_PORT || process.env.FTP_PORT || '21', 10),
    user,
    pass: process.env.UPLOAD_DEFAULT_FTP_PASS || process.env.FTP_PASS || '',
    dir: (process.env.UPLOAD_DEFAULT_FTP_DIR || process.env.FTP_DIR || '').replace(/\/+$/, ''),
    coverPath: (process.env.UPLOAD_DEFAULT_COVER_PATH || '').replace(/\/+$/, ''),
    urlPrefix: (process.env.UPLOAD_DEFAULT_URL_PREFIX || process.env.FTP_URL_PREFIX || '').replace(/\/+$/, ''),
    timeoutMs: parseInt(process.env.UPLOAD_DEFAULT_FTP_TIMEOUT_MS || process.env.FTP_TIMEOUT_MS || '360000', 10),
  };
}

// 默认目标的展示名（配了才回，否则 null）。
function defaultTargetLabel() {
  const c = defaultTargetConfig();
  return c ? c.label : null;
}

// 已配置好的目标列表（只回 key/label，不泄露 FTP 凭证）。
// 目标 = logo 名。优先用显式 UPLOAD_TARGETS；没配则【自动发现】：
// 扫 env 里所有 UPLOAD_<KEY>_FTP_HOST，KEY 小写即目标名（= 对应 logo 名）。
// 所以只要配了 UPLOAD_VODVIP_FTP_HOST 那组，vodvip logo 就自动能上传，无需 UPLOAD_TARGETS。
function listUploadTargets() {
  let keys = String(process.env.UPLOAD_TARGETS || '')
    .split(',').map((s) => s.trim()).filter(Boolean);
  if (!keys.length) {
    keys = Object.keys(process.env)
      .map((k) => /^UPLOAD_(.+)_FTP_HOST$/.exec(k))
      .filter(Boolean)
      .map((m) => m[1].toLowerCase());
  }
  const seen = new Set();
  return keys
    .filter((k) => k !== 'default' && (seen.has(k) ? false : (seen.add(k), true))) // default 单独作兜底，不进 logo 专属列表
...
module.exports = { uploadCover, ftpStore, listUploadTargets, uploadCoverToTarget, uploadTargetConfig, defaultTargetLabel };
