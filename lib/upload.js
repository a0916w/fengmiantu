...
function rsyncStore(buffer, filename, cfg) {
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.promises.writeFile(tmp, buffer).then(() => {
      ...
      // RSYNC_SUDO=1：以 sudo 跑，让 rsync 用 root 身份（可读 root 属主的密码文件）。
      // -E 保留环境变量(RSYNC_PASSWORD 走 env 时需要)；需 sudoers 给 NOPASSWD。
      const bin = cfg.sudo ? 'sudo' : 'rsync';
      const spawnArgs = cfg.sudo ? ['-E', 'rsync', ...args] : args;
