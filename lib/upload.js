...
function publishRsyncConfig(logo) {
  ...
  return {
    ...
    passwordFile: pick('PASSWORD_FILE'),                 // 密码文件路径(如 /etc/mm_caiji.pwd)；配了就用 --password-file 代替 RSYNC_PASSWORD
    sudo: /^(1|true|yes)$/i.test(pick('SUDO')),          // RSYNC_SUDO=1 → 用 sudo rsync 跑(读 root 属主的密码文件等)；需 NOPASSWD sudoers
...
function rsyncStore(buffer, filename, cfg) {
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.promises.writeFile(tmp, buffer).then(() => {
      ...
      const args = ['--contimeout', String(contimeout), '--timeout', String(Math.max(1, Math.round(timeoutMs / 1000)))];
      // 认证：配了 RSYNC_PASSWORD_FILE 用 --password-file(该文件需 chmod 600)，否则走 RSYNC_PASSWORD 环境变量。
      if (cfg.passwordFile) args.push('--password-file', cfg.passwordFile);
      args.push(tmp, dest);
      if (cfg.port && cfg.port !== 873) args.unshift('--port', String(cfg.port));
      const env = { ...process.env };
      if (!cfg.passwordFile && cfg.pass) env.RSYNC_PASSWORD = cfg.pass;
      // RSYNC_SUDO=1：以 sudo 跑，让 rsync 用 root 身份（可读 root 属主的密码文件）。需 NOPASSWD sudoers。
      // 用密码文件时不需要 -E（root 直接读文件）；只有走 RSYNC_PASSWORD 环境变量时才 -E 保留 env
      // （-E 还需 sudoers 允许 SETENV，能不用就不用）。
      const bin = cfg.sudo ? 'sudo' : 'rsync';
      const spawnArgs = cfg.sudo
        ? [...(cfg.passwordFile ? [] : ['-E']), 'rsync', ...args]
        : args;
      const ps = spawn(bin, spawnArgs, { env });
      let stderr = '';
